...
export class CachingService {
	 ...
	 */
	async getOrSet<T>(key: string, fetchDataFn: () => Promise<T>, ttlSeconds: number): Promise<T | null> {
		try {
			// 1. Try cache
			...
			if (freshData !== null && freshData !== undefined) { // Avoid caching null/undefined explicitly
				try {
