...
async function main() {
  ...
  if (args.length < 2) {
    console.log("Usage: npx hardhat run scripts/deploy-perk-collection.ts --network <network> -- <name> <symbol> [category] [maxSupply]");
    console.log("Example: npx hardhat run scripts/deploy-perk-collection.ts --network soneiumTestnet -- 'Harajiri Falls Collection' 'HARAJIRI' 'waterfalls' 1000");
    process.exit(1);
  }
  ...
  const deploymentsPath = join(__dirname, "..", "deployments", `${network.name}-${network.chainId}.json`);
  ...
  if (network.chainId !== 31337n) {
    console.log("\n🔍 Verification command:");
    console.log(`npx hardhat verify --network ${network.name} ${perkAddress} "${name}" "${symbol}"`);
  }
}
