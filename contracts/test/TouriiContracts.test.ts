...
describe('Tourii Smart Contracts', () => {
    ...
    describe('TouriiDigitalPassport', () => {
        it('Should deploy and have correct name', async () => {
        ...
        it('Should mint passport to user', async () => {
            ...
            expect(await digitalPassport.tokenURI(0)).to.equal(
                'https://api.tourii.com/api/passport/metadata/0.json',
            );
        ...
        it('Should prevent minting multiple passports to same address', async () => {
            const { digitalPassport, addr1 } = await loadFixture(deployContractsFixture);

            await digitalPassport.mint(addr1.address);

            await expect(digitalPassport.mint(addr1.address)).to.be.revertedWith(
                'Address already has a passport',
            );
        });
        ...
        it('Should update base URI', async () => {
            ...
            expect(await digitalPassport.tokenURI(0)).to.equal(
                'https://newapi.tourii.com/api/passport/metadata/0.json',
            );
    ...
    describe('TouriiPerk', () => {
        ...
        it('Should mint perk to user', async () => {
        ...
        it('Should allow perk redemption', async () => {
            const { perk, addr1 } = await loadFixture(deployContractsFixture);
    ...
    describe('TouriiLog', () => {
        ...
        it('Should mint log with category', async () => {
        ...
        it('Should track user logs', async () => {
            const { log, addr1 } = await loadFixture(deployContractsFixture);
