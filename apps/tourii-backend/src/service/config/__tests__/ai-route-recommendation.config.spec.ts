import { AI_ROUTE_CONFIG, getEnvironmentConfig, getAiRouteConfig } from '../ai-route-recommendation.config';
...
describe('AI Route Recommendation Config', () => {
  ...
  describe('Configuration Validation', () => {
    it('should have consistent proximity radius limits', () => {
      const { MIN_PROXIMITY_RADIUS_KM, MAX_PROXIMITY_RADIUS_KM, DEFAULT_PROXIMITY_RADIUS_KM } = 
    ...
    it('should have consistent keyword limits', () => {
      const { MIN_KEYWORD_LENGTH, MAX_KEYWORD_LENGTH, MAX_KEYWORDS } = 
        AI_ROUTE_CONFIG.SEARCH;
    ...
    it('should have valid OpenAI parameters', () => {
      const { MAX_TOKENS, TEMPERATURE, MAX_RETRIES, REQUEST_TIMEOUT_MS, MIN_CONFIDENCE_SCORE } = 
    ...
    it('should have consistent rate limiting values', () => {
      const { WINDOW_MS, MAX_REQUESTS_PER_WINDOW, MAX_ANONYMOUS_REQUESTS, 
              AI_GENERATION_LIMIT_PER_HOUR, RATE_LIMIT_COOLDOWN_SECONDS } = 
    ...
    it('should have non-empty default values', () => {
      const { DEFAULT_REGION_DESC, FALLBACK_ROUTE_NAME, DEFAULT_ESTIMATED_DURATION,
              DEFAULT_BACKGROUND_IMAGE, FALLBACK_RECOMMENDATIONS } = 
    ...
    it('should have consistent validation limits', () => {
      const { MAX_ROUTE_NAME_LENGTH, MAX_REGION_DESC_LENGTH, MAX_RECOMMENDATIONS_COUNT, 
              MIN_RESULT_CONFIDENCE } = AI_ROUTE_CONFIG.VALIDATION;
  ...
  describe('Feature Flags', () => {
    ...
    it('should have consistent feature defaults', () => {
      const { AI_GENERATION_ENABLED, CACHING_ENABLED, RATE_LIMITING_ENABLED,
              PERFORMANCE_MONITORING_ENABLED, FALLBACK_MODE_ENABLED } = 
  ...
  describe('Configuration Immutability', () => {
    ...
    it('should maintain reference equality for repeated calls', () => {
      ...
      expect(config1.CLUSTERING.DEFAULT_PROXIMITY_RADIUS_KM)
        .toBe(config2.CLUSTERING.DEFAULT_PROXIMITY_RADIUS_KM);
    });
  });
  ...
  describe('Environment Variable Integration', () => {
    it('should support different environments simultaneously', () => {
      ...
      try {
        ...
        expect(devConfig.RATE_LIMITING.MAX_REQUESTS_PER_WINDOW)
          .not.toBe(prodConfig.RATE_LIMITING.MAX_REQUESTS_PER_WINDOW);
