...
async function main() {
	...
	// --- 1. Seed Master Data ---
	console.log("Seeding discord_roles...");
	const roleUser = await prisma.discord_roles.upsert({
		where: { role_id: BigInt("100000000000000001") }, // Use BigInt constructor
		create: {
	...
	const roleModerator = await prisma.discord_roles.upsert({
		where: { role_id: BigInt("100000000000000002") },
		create: {
	...
	const roleAdmin = await prisma.discord_roles.upsert({
		where: { role_id: BigInt("100000000000000003") },
		create: {
	...
	console.log("Seeded discord_roles.");

	console.log("Seeding level_requirement_master...");
	await prisma.level_requirement_master.upsert({
		where: { level: LevelType.BONJIN },
		create: {
	...
	await prisma.level_requirement_master.upsert({
		where: { level: LevelType.E_CLASS_AMATSUKAMI },
		create: {
	...
	console.log("Seeded level_requirement_master.");

	console.log("Seeding kendama_random_range...");
	await prisma.kendama_random_range.upsert({
		where: { season: KendamaSeason.NORMAL },
		create: {
	...
	await prisma.kendama_random_range.upsert({
		where: { season: KendamaSeason.EVENT },
		create: {
	...
	console.log("Seeded kendama_random_range.");

	console.log("Seeding onchain_item_catalog...");
	const catalogItemPassport = await prisma.onchain_item_catalog.upsert({
		where: { onchain_item_id: "CATALOG-PASSPORT-001" }, // Using a custom unique ID for upsert
		create: {
	...
	const catalogItemPerk = await prisma.onchain_item_catalog.upsert({
		where: { onchain_item_id: "CATALOG-PERK-001" },
		create: {
			...
			attributes: [{ trait_type: "Discount", value: "10%" }],
	...
	const catalogItemLogNFT = await prisma.onchain_item_catalog.upsert({
		where: { onchain_item_id: "CATALOG-LOGNFT-001" },
		create: {
			...
			attributes: [{ trait_type: "Saga", value: "Prologue" }],
			release_date: new Date("2024-01-01T00:00:00Z"),
			max_supply: 0, // Unlimited
			ins_user_id: "system-seed",
		},
	...
	console.log("Seeded onchain_item_catalog.");
	...
	// --- 3. Seed User-related Info & Logs ---
	console.log("Seeding user_info...");
	await prisma.user_info.upsert({
		where: { user_id: userAlice.user_id },
		create: {
	...
	await prisma.user_info.upsert({
		where: { user_id: userBobAdmin.user_id },
		create: {
	...
	console.log("Seeded user_info.");

	console.log("Seeding user_achievement...");
	...
	console.log("Seeded user_achievement.");

	console.log("Seeding user_onchain_item...");
	...
	console.log("Seeded user_onchain_item.");

	console.log("Seeding user_item_claim_log...");
	...
	console.log("Seeded user_item_claim_log.");

	console.log("Seeding discord_user_roles...");
	...
	console.log("Seeded discord_user_roles.");

	console.log("Seeding discord_activity_log...");
	...
	console.log("Seeded discord_activity_log.");

	console.log("Seeding discord_rewarded_roles...");
	...
	console.log("Seeded discord_rewarded_roles.");

	console.log("Seeding user_invite_log...");
	...
	console.log("Seeded user_invite_log.");

	// --- 4. Seed Gamification Structure ---
	console.log("Seeding story (Saga)...");
	...
	console.log("Seeded story (Saga).");

	console.log("Seeding model_route...");
	...
	console.log("Seeded model_route.");

	console.log("Seeding tourist_spot...");
	...
	console.log("Seeded tourist_spot.");

	console.log("Seeding story_chapter...");
	const chapterPrologue1 = await prisma.story_chapter.create({
		data: {
	...
	console.log("Seeded story_chapter.");

	console.log("Seeding quest...");
	...
	console.log("Seeded quest.");

	console.log("Seeding quest_task...");
	const taskHarajiriUpload = await prisma.quest_task.create({
		data: {
			...
			anti_cheat_rules: { claim_once: true }, // JSON object
			magatama_point_awarded: 25,
			ins_user_id: "system-seed",
		},
	});
	const taskCaveQuestion1 = await prisma.quest_task.create({
		data: {
			...
			select_options: [
				...
				{ option_text: "100 years", answer_flag: false },
				{ option_text: "500 years", answer_flag: false },
				{ option_text: "Over 1000 years", answer_flag: true },
			],
			group_activity_members: [],
			anti_cheat_rules: { max_attempts: 1 },
			magatama_point_awarded: 25,
			ins_user_id: "system-seed",
		},
	});
	const taskCaveQuestion2 = await prisma.quest_task.create({
		data: {
			quest_id: questCaveQuiz.quest_id,
			task_theme: TaskTheme.LOCAL_CULTURE,
			...
			select_options: [],
			group_activity_members: [],
			anti_cheat_rules: { only_once: true },
			magatama_point_awarded: 25,
			ins_user_id: "system-seed",
		},
	});
	console.log("Seeded quest_task.");

	// --- 5. Seed User Gamification Logs ---
	console.log("Seeding user_story_log...");
	...
	console.log("Seeded user_story_log.");

	console.log("Seeding user_quest_log...");
	...
	console.log("Seeded user_quest_log.");

	console.log("Seeding user_travel_log...");
	...
	console.log("Seeded user_travel_log.");
