...
function normalizeHashtag(hashtag: string): string {
    ...
    let cleaned = hashtag.replace(/^#/, '').trim();
    ...
    return splitCompound.split(' ').map(word => 
...
async function standardizeHashtags() {
    ...
    try {
        ...
        for (const spot of spots) {
            ...
            const hasChanges = originalHashtags.length !== standardizedHashtags.length ||
        ...
        console.log(`   • Hashtags removed (duplicates): ${totalHashtagsBefore - totalHashtagsAfter}`);
