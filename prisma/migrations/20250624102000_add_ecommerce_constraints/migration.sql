...
CREATE INDEX CONCURRENTLY "idx_user_order_currency" ON "user_order"("currency");
CREATE INDEX CONCURRENTLY "idx_user_order_payment_method" ON "user_order"("payment_method");
CREATE INDEX CONCURRENTLY "idx_user_order_customer_email" ON "user_order"("customer_email") WHERE "customer_email" IS NOT NULL;

-- Add partial indexes for better query performance
CREATE INDEX CONCURRENTLY "idx_user_cart_active" ON "user_cart"("user_id", "added_at") WHERE "del_flag" = false;
CREATE INDEX CONCURRENTLY "idx_user_order_active" ON "user_order"("user_id", "order_date") WHERE "del_flag" = false;
CREATE INDEX CONCURRENTLY "idx_user_order_item_active" ON "user_order_item"("order_id") WHERE "del_flag" = false;