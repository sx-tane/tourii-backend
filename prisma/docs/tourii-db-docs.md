...
"tourist_spot" {
  ...
  String model_route_id FK "nullable"
...
**Properties**
  ...
  - `is_ai_generated`
    > Flag to indicate if this route was generated by AI system
    > True for AI-generated routes, false for manually created routes
  - `del_flag`: Soft delete flag
  - `ins_user_id`: ID of user who created this record
  - `ins_date_time`: Timestamp of record creation
  - `upd_user_id`: ID of user who last updated this record
...
**Properties**
  ...
  - `model_route_id`: ID of the associated travel route (optional for shared spots)
...
### `route_tourist_spot`
Junction table for many-to-many relationship between routes and tourist spots

This table allows tourist spots to be shared across multiple routes,
especially useful for AI-generated routes that reference existing locations.
The display_order field allows controlling the order of spots within a route.

**Properties**
  - `model_route_id`: ID of the model route
  - `tourist_spot_id`: ID of the tourist spot
  - `display_order`: Display order of the spot within the route (1-based)
  - `is_primary`: Whether this is the primary route for the spot (for backwards compatibility)
  - `created_at`: Timestamp of record creation
  - `created_by`: ID of user who created this association
